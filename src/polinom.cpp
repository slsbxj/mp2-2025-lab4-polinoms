...
Polinom::Polinom(Polinom &&other) noexcept
...
void Polinom::InsertSorted(const Monom &monom) {
  if (std::abs(monom.coefficient) < 1e-10)
    return;
...
Polinom Polinom::operator*(const Polinom &other) const {
  ...
  while (current1) {
    ...
    while (current2) {
      ...
      Monom newMonom(current1->data.coefficient * current2->data.coefficient,
                     newX * 100 + newY * 10 + newZ);
      result.InsertSorted(newMonom);
...
Polinom Polinom::operator-() const { return *this * (-1.0); }

void Polinom::mergePolinom(const Polinom &a, const Polinom &b, Polinom &result,
                           double sign) {
...
std::ostream &operator<<(std::ostream &os, const Polinom &p) {
  ...
  while (current) {
    ...
    if (x > 0)
      os << "x" << (x > 1 ? "^" + std::to_string(x) : "");
    if (y > 0)
      os << "y" << (y > 1 ? "^" + std::to_string(y) : "");
    if (z > 0)
      os << "z" << (z > 1 ? "^" + std::to_string(z) : "");
